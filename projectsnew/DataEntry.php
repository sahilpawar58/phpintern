            ...
            if (isset($_POST['submit'])) {
                ...
                if (!empty($_FILES["formFileMultiple"]["name"])) {
                    ...
                    if (in_array($fileType, $allowTypes)) {
                        $image = $_FILES['formFileMultiple']['tmp_name'];
                        $imgContent = ((file_get_contents($image)));
                        echo $_FILES["formFileMultiple"]["name"];
                        // Insert image content into database 
                        $insert = $con->query("update files set  sem" . $_SESSION['sem_no'] . " = \"" . $imgContent . "\" where sid=" . $_SESSION['id']);
                        echo  $insert;
                        if ($insert) {
                            $status = 'success';
