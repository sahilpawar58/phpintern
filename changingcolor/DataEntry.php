...
<body>
    ...
    <form method="post">
        <header class="header-container">
            ...
            <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups"
                ...
                <div class="btn-group me-2" role="group" aria-label="First group">
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(1); ?>"
                        name="sem1">1</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(2); ?>"
                        name="sem2">2</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(3); ?>"
                        name="sem3">3</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(4); ?>"
                        name="sem4">4</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(5); ?>"
                        name="sem5">5</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(6); ?>"
                        name="sem6">6</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(7); ?>"
                        name="sem7">7</button>
                    <button type="submit" class="btn btn-primary" style="<?php echo semcolor(8); ?>"
                        name="sem8">8</button>
                </div>
            </div>
            <div class="col-md-6">
        ...
        function semcolor($semno)
        {
            $con = new mysqli('localhost', 'root', '', 'transcriptDB');
            if ($con) {


                $sems = mysqli_fetch_assoc($con->query('SELECT subject FROM `warehouse` WHERE sem_no=' . $semno . ' and sid="' . $_SESSION['id'] . '";'));

                if (isset($sems['subject'])) {
    ...
    <div class="table-container">
        <form method="post" enctype="multipart/form-data">
            <div class="m1">
                ...
                <table class="table table-dark" style="text-align: center; border: 2px solid gray; margin-top: 10px;">
                    ...
                    <tbody>
                        ...
                        $con = new mysqli('localhost', 'root', '', 'transcriptDB');
                        if ($con) {
                            if (semcolor($_SESSION['sem_no']) == "background-color:red;") {
                        ...
                        <?php } ?>
                        <?php } else {
                                $scheme = mysqli_fetch_assoc($con->query('SELECT scheme FROM warehouse WHERE sid = "' . $_SESSION['id'] . '" AND sem_no = ' . $_SESSION['sem_no'] . ';'));
                                $student_details = mysqli_fetch_assoc($con->query('SELECT name, department FROM students WHERE sid = "' . $_SESSION['id'] . '";'));
                                $dept = mysqli_fetch_assoc($con->query('SELECT department FROM students WHERE sid = "' . $_SESSION['id'] . '";'));
                                $scheme_id = mysqli_fetch_assoc($con->query('SELECT scheme_id FROM schemes WHERE scheme_name = "' . $scheme['scheme'] . '";'));
                                $dept_id = mysqli_fetch_assoc($con->query('SELECT dept_id FROM departments WHERE dept_name = "' . $dept['department'] . '";'));
                                $subjects = $con->query('SELECT subject_name FROM subjects WHERE scheme_id = ' . strval($scheme_id['scheme_id']) . ' AND dept_id = ' . strval($dept_id['dept_id']) . ' AND sem_no = ' . $_SESSION['sem_no'] . ';');
                                $marks = $con->query('SELECT * FROM warehouse WHERE sid = "' .  $_SESSION['id'] . '" AND sem_no = ' . strval($_SESSION['sem_no']) . ';');
                                $i = 0;
                                while ($row = $subjects->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['ese'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['ia_tw'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['overall'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['credits_earned'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['credits_pts'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                            <td style="border: 1px solid gray;">
                                <input type="number" disabled value="<?php echo $col['cxg'] ?>"
                                    name="-<?php echo str_replace(" ",  "", $col['subject']) ?>-[]" />
                            </td>
                        </tr>
                        <?php } ?>
            ...
            <?php if (semcolor($_SESSION['sem_no']) == "background-color:red;") { ?>
